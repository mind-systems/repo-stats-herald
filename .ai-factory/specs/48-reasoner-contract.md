...
- Write red tests over mocked `Embedder`/`KnowledgeStore`/`EpisodicStore`/`LLMClient` pinning:
  ...
  - **`repo`-scoping** — calling `answer(query, repo="o/r")` passes `repo="o/r"` to both stores' `query`; calling `answer(query)` (no `repo`) passes `repo=None` to both — never silently defaulting to org-wide when a repo was given, or vice versa;
...
- Each of the three pinned invariants (single embedding reused across both stores, correct `repo`-scoping passed through, structurally distinct no-memory prompt path) has a corresponding red test.
