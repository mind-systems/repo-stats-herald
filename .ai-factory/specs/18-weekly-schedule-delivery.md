...
- `scripts/report.py --schedule <name>` — a composition-root entrypoint: look up the schedule in `Settings.report_schedules` (10.1.1) → its window + ordered sections → assemble a `Report`; for each `(repo, org_id)` in the **`served_repos` table (3.5)**, `Report.build(repo, org_id)`, and where it returns text, resolve a delivery plan and `DeliveryService.deliver(plan, report)` (Phase 9) to the org's Telegram channel.
...
- Assembly only — iterate → build → resolve-plan → deliver; the report shape lives in config, not here.
