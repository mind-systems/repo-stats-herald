...
Implement three sections; each is a self-contained content block over the report's resolved `(before, after)` range and `lang` (both from `render`, 10.1.1).
...
- `PerBranchSection(ReportSection)` — the **per-branch** block: enumerate branches active in the window. A single canonical-ref `(before, after)` SHA range names no other branch's commits, so the section derives the window's **time boundary** from the `before`/`after` commit dates (`git show -s --format=%cI`) and finds each branch with commits in that span; for each, resolve its change and narrate who-did-what (`Reasoner.narrate(change, lang)`); one labelled sub-part per branch; none active → `None`.
- `RemainingSection(ReportSection)` — the **what-remains** block: read the currently-open `[ ]` tasks at the window's end commit **`after`** (via the source strategy's roadmap path) — `after` is the report's "now", not literal repo HEAD, which on a bare mirror may be a different ref's tip — and frame them; no open tasks → `None`.
