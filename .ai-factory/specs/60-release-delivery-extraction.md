...
- The defensive attribute lookup that reads the changelog base URL with a fallback default is carried over verbatim; its tension with the delivery-plan contract that is supposed to fill that field on every resolution is a separate, later decision and is explicitly not resolved by this task.
