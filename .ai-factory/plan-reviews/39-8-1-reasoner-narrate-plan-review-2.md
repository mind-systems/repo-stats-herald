...
- Affects: Phase 3 / user-authored eval data (`evals/cases.yaml`, `evals/reference/`) — Task 3 registers the `narrate` handler but adds no `narrate` case to `cases.yaml` and no reference note, so the harness verification stays latent until a user authors a `{repo, range, lang}` case and its reference. This is correct per the project's "references user-authored, never fabricated" rule (CLAUDE.md) and lies outside this code task's scope; noted only so the eval linkage isn't assumed live on merge.
