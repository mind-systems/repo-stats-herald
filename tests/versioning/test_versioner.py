...
async def test_no_tags_first_staging_push_is_v0_1_0_rc(git_repo, collector):
    ...
    result = await versioner.next("org/repo", BranchRole.STAGING, base_sha, staging_sha)

    assert result == Version(0, 1, 0, prerelease=True)


async def test_no_tags_default_branch_push_with_no_candidate_is_v0_1_0(git_repo, collector):
    base_sha = _commit("base", cwd=git_repo)
    versioner = _versioner(git_repo, collector)

    result = await versioner.next("org/repo", BranchRole.RELEASE, base_sha, base_sha)

    assert result == Version(0, 1, 0)


async def test_two_successive_staging_pushes_each_advance_off_the_semver_max(git_repo, collector):
    base_sha = _commit("base", cwd=git_repo)
    ...
    first = await versioner.next("org/repo", BranchRole.STAGING, base_sha, first_sha)
    ...
    second = await versioner.next("org/repo", BranchRole.STAGING, first_sha, second_sha)

    assert second == Version(1, 2, 2, prerelease=True)


async def test_promotion_adopts_reachable_rc_candidate_base_with_no_further_bump(git_repo, collector):
    base_sha = _commit("base", cwd=git_repo)
    _checkout_new_branch(git_repo, "staging", base_sha)
    staging_sha = _commit("staging work", cwd=git_repo)
    ...
    result = await versioner.next("org/repo", BranchRole.RELEASE, base_sha, after_sha)

    assert result == Version(1, 3, 0)


async def test_hotfix_bumps_off_last_full_release_when_no_reachable_candidate(git_repo, collector):
    base_sha = _commit("base", cwd=git_repo)
    _tag(git_repo, "v1.2.0", base_sha)
    hotfix_sha = _commit("hotfix", cwd=git_repo)
    versioner = _versioner(git_repo, collector)

    result = await versioner.next("org/repo", BranchRole.RELEASE, base_sha, hotfix_sha)

    assert result == Version(1, 2, 1)


async def test_fast_forward_back_merge_into_staging_returns_none(git_repo, collector):
    base_sha = _commit("base", cwd=git_repo)
    _checkout_new_branch(git_repo, "staging", base_sha)
    _checkout(git_repo, _TRUNK)
    ...
    result = await versioner.next("org/repo", BranchRole.STAGING, base_sha, after_sha)

    assert result is None


async def test_merge_commit_back_merge_round_trip_of_a_released_hotfix_returns_none(git_repo, collector):
    ...
    result = await versioner.next("org/repo", BranchRole.STAGING, base_sha, merge_sha)
